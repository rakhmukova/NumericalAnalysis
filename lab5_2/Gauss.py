...
class GaussIntegration:
    ...
    def calc_coefficients(self, polynomials, nodes):
        ...
        for i in range(self.N):  # self.N
            ...
            # print(f'node {node[0]} poly {polynom(node[0])}')
    ...
    def integrate_for_custom_borders(self, a, b, degree, polynomials_nodes, coefficients):
        ...
        for i in range(degree):
            ...
            sum_int += custom_coefficients[i] * self.f(custom_nodes[i])
        tabulate_results(zip(custom_nodes, custom_coefficients), headers=['узел', 'коэфф'],
                         title=f'Узлы и коэффициенты КФ Гаусса для N={degree}')
        return sum_int
    ...
    def integrate_custom(self, polynomials_nodes, coefficients):
        ...
        for degree in self.degrees:
            approximate_value = self.integrate_for_custom_borders(a, b, degree, polynomials_nodes, coefficients)
